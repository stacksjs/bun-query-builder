...
describe('ModelInstance helper methods', () => {
  ...
  it('getAttribute reads the raw value, skipping computed accessors', async () => {
  ...
  it('get() falls back to raw attribute when accessor returns undefined', async () => {
    const post = (await Post.find(1))!
  ...
  it('get() catches exceptions thrown by buggy accessors', async () => {
    const post = (await Post.find(1))!
  ...
  it('getAttributes returns a shallow copy of all attributes', async () => {
    const post = (await Post.find(1))!
  ...
  it('only() narrows to the named attributes', async () => {
    const post = (await Post.find(1))!
  ...
  it('except() drops the named attributes', async () => {
    const post = (await Post.find(1))!
  ...
  it('toArray() returns the attribute snapshot', async () => {
    const post = (await Post.find(1))!
  ...
  it('refresh() updates this instance in place and clears dirty state', async () => {
    const post = (await Post.find(1))!
    post.set('views', 1234)
    expect(post.isDirty()).toBe(true)
    const same = await post.refresh()
  ...
  it('fresh() returns null when the row no longer exists', async () => {
    const post = (await Post.find(1))!
    getDatabase().run(`DELETE FROM test_helpers_posts WHERE id = ?`, [1])
    expect(await post.fresh()).toBeNull()
  })

  it('refresh() returns null when the row no longer exists', async () => {
    ...
    const post = (await Post.find(99))!
    db.run(`DELETE FROM test_helpers_posts WHERE id = ?`, [99])
    expect(await post.refresh()).toBeNull()
  })
})
